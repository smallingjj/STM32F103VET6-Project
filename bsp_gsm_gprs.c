...
uint8_t gsm_sms(char *num,char *smstext)
{
    ...
    if(IsASSIC(smstext)==GSM_TRUE)					//判断是英文还是中文
    {
			//英文
			//"GSM"字符集
...
const char num[]="13322887972";											//设置电话号码！！！


char namenum[20*4]={0},str[512]={0},gbkstr[256]={0},namegbk[256]={0};		//定义了四个字符数组，用于存储短信内容和发送者信息。
...
uint8_t newmessadd=0,IsRead=0;			//用于存储新短信的地址和是否已读的标志
uint8_t testCard = 0;								//定义了一个uint8_t类型的变量testCard，用于检测电话卡是否插入的计数器。
...
void GSM_Init(void)
{
    ...
    //检测模块响应是否正常
    while(gsm_cmd("AT\r","OK",1000) != GSM_TRUE)			//检测模块响应是否正常，如果不正常会输出相应提示信息
    {
      printf("\r\n模块响应测试不正常！！\r\n");
	...
	printf("\r\n正在等待GSM模块初始化...\r\n");
	while(IsInsertCard() != GSM_TRUE)								//检测电话卡是否插入，如果未检测到会输出相应提示信息。
	{
...
{
			case 1:
			sprintf(sms_message, "\r\n温度过高\r\n温度：%d℃，湿度：%d％ ，有害气体浓度：%u%％", show_tmp, show_humi, smokeDensity/41);
//			printf(sms_message);
			gsm_sms((char *)num,sms_message);
			break;
			case 2:
			sprintf(sms_message, "\r\n湿度过高\r\n温度：%d℃, 湿度：%d％,有害气体浓度：%u％", show_tmp, show_humi, smokeDensity/41);					//%d表示整形，
//			printf(sms_message);
			gsm_sms((char *)num,sms_message);	
			break;
